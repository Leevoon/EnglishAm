...
const IeltsComplete = () => {
  ...
  const loadCompleteTest = async () => {
    try {
      setLoading(true);
      const response = await ieltsAPI.getComplete();
      setTestData(response.data);
    } catch (error) {
      console.error('Error loading complete IELTS test:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return <div className="ielts-section">Loading complete IELTS test...</div>;
  }

  if (!testData) {
    return <div className="ielts-section">No test data available</div>;
  }

  return (
    <div className="ielts-section">
      <div className="container">
        <h1>Complete IELTS Test</h1>
